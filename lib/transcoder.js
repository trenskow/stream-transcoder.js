...
function Transcoder(source) {
	...
	Transcoder.prototype._exec = function(a) {
		...
		child.on('exit', function(code) {
			if (!code) self.emit('finish');
				...
				self.emit('error', new Error('FFmpeg error: ' + self.lastErrorLine));
	...
	Transcoder.prototype.videoFormat = function(format) {
	...
	Transcoder.prototype.maxSize = function(width, height) {
		var flt = 'scale=min(trunc(' + width + '/hsub)*hsub\\,trunc(a*' + height + '/hsub)*hsub):' +
						'min(trunc(' + height + '/vsub)*vsub\\,trunc(' + width + '/a/vsub)*vsub)';
		this.args['vfscale'] = [ '-vf', flt ];
		return this;
	};
	
	/* Set minimum video size. Adjusts size to maintain aspect ratio, making it grow to size. */
	Transcoder.prototype.minSize = function(width, height) {
		var flt = 'scale=max(trunc(' + width + '/hsub)*hsub\\,trunc(a*' + height + '/hsub)*hsub):' +
	...
	Transcoder.prototype.audioCodec = function(format) {
		this.args['acodec'] = [ '-acodec', format ];
		return this;
	};
	...
	Transcoder.prototype.captureFrame = function(time) {
		...
		return this.videoCodec('mjpeg').videoFormat('mjpeg');
